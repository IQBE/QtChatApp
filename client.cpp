...
Client::Client() : peerManager(new PeerManager(this)) {
...
bool Client::hasConnection(const QHostAddress &senderIp, int senderPort) const {
    ...
    if (begin == peers.constEnd()) return false;

    if (senderPort == -1) return true;
...
void Client::disconnected() {
    if (Connection *connection = qobject_cast<Connection *>(sender())) removeConnection(connection);
}
...
void Client::removeConnection(Connection *connection) {
    if (peers.remove(connection->peerAddress(), connection) > 0) {
        QString nick = connection->name();
        if (!nick.isEmpty()) emit participantLeft(nick);
    }
    connection->deleteLater();
}

Client::~Client() {
    disconnect(peerManager, &PeerManager::newConnection, this, &Client::newConnection);
}
